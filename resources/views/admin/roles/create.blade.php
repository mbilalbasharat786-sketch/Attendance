    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight flex items-center">
                <i class="fa-solid fa-circle-plus mr-3 text-indigo-600"></i>
                {{ __('Create New Role') }}
            </h2>
            <a href="{{ route('admin.roles.index') }}" class="text-sm bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 px-4 py-2 rounded-lg transition shadow-sm flex items-center">
                ...
                <form action="{{ route('admin.roles.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="name" required class="w-full pl-10 pr-3 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 bg-slate-50 transition font-medium" placeholder="e.g. Teacher, HR, Manager">
                        </div>
                        @error('name')
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-4 uppercase tracking-wider flex items-center">
                            <i class="fa-solid fa-list-check mr-2 text-indigo-500"></i> Assign Permissions
                        </label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @forelse($permissions as $permission)
                                    <label class="flex items-center space-x-3 cursor-pointer group p-2 hover:bg-white rounded-md transition border border-transparent hover:border-slate-200 hover:shadow-sm">
                                        <input type="checkbox" name="permissions[]" value="{{ $permission->name }}" class="w-5 h-5 text-indigo-600 bg-white border-slate-300 rounded focus:ring-indigo-500 focus:ring-2">
                                ...
                                @empty
                                    <div class="col-span-full text-center py-4 text-slate-500 italic text-sm">
                                        <i class="fa-solid fa-triangle-exclamation mr-2 text-amber-500"></i> No permissions available in the system yet. You can still create the role.
                                    </div>
                                @endforelse
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end pt-4">
                        <button type="submit" class="bg-indigo-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-200 transition duration-150 shadow-md flex items-center">
                            <i class="fa-solid fa-floppy-disk mr-2"></i> Save Role
                        </button>
                    </div>
                </form>
            </div>
